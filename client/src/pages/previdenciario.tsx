...
function naturezaLabel(n: NaturezaVinculo) {
  const m: Record<string,string> = {
    ...
    BENEFICIO_INCAPACIDADE:"BenefÃ­cio Incapacidade", NAO_INFORMADO:"NÃ£o informado",
  };
  return m[n] || n;
}
...
async function extrairTextoPdf(file: File): Promise<string> {
  if (!(window as any).pdfjsLib) throw new Error("PDF.js nÃ£o carregado");
...
// ExtraÃ§Ã£o ordenada por coordenadas YâX (resolve o problema de colunas misturadas)
...
function parsearDtcCtc(texto: string): DtcCtcData {
  ...
  const extrairAnos = (ln: string): (number | null)[] => {
    const m = ln.match(/Ano:\s*[-â]|\bAno:\s*(\d{4})/gi) || [];
  ...
  for (let i = 0; i < linhas.length; i++) {
    ...
    // Captura dados bÃ¡sicos
    if (!nome && /NOME DO SERVIDOR|NOME:/i.test(l)) {
      const m = l.match(/(?:NOME DO SERVIDOR|NOME)[\s:]*([A-ZÃÃÃÃÃÃÃÃÃÃÃÃÃ][A-ZÃÃÃÃÃÃÃÃÃÃÃÃÃ\s]+)/i);
      if (m) nome = m[1].trim();
    }
    if (!cpf && /CPF/i.test(l)) {
    ...
    if (!orgao && /ÃRGÃO EXPEDIDOR|PREFEITURA|SECRETARIA/i.test(l)) {
      const m = l.match(/(?:ÃRGÃO EXPEDIDOR|PREFEITURA MUNICIPAL DE)[:\s]*([^\n|]+)/i);
      if (m && m[1].trim().length > 3) orgao = m[1].trim();
    }

    // PerÃ­odos (Anexo IV / PerÃ­odo de ContribuiÃ§Ã£o)
    if (/PERÃODO.*TEMPO DE CONTRIBUIÃÃO|QUADRO I/i.test(l)) { capturandoPer = true; continue; }
    if (capturandoPer && (/DADOS DE REMUNERAÃÃES|RELAÃÃO DAS REMUNERAÃÃES|ANEXO V|QUADRO II/i.test(l))) {
      capturandoPer = false;
    }
    if (capturandoPer) {
    ...
    // RemuneraÃ§Ãµes (Anexo V / RelaÃ§Ã£o de RemuneraÃ§Ãµes)
    if (/DADOS DE REMUNERAÃÃES.*ANEXO V|RELAÃÃO DAS REMUNERAÃÃES/i.test(l)) {
      capturandoRem = true; anosHeader = []; continue;
    }
    if (capturandoRem && /^ASSINATURA|^UNIDADE GESTORA|^OBSERVAÃÃES|^HOMOLOGO|Lavrei/i.test(l)) {
    ...
    // Linha de mÃªs/valores â suporta tanto formato com | quanto formato de colunas direto
    const parts = l.split(" | ").map(s => s.trim());
    if (parts.length >= 2) {
      const mesStr = parts[0].toUpperCase().replace(/[^A-ZÃÃÃÃÃÃÃÃÃ]/g, "");
      const mesIdx = MESES_PT.findIndex(m => mesStr.startsWith(m.slice(0, 3)));
      if (mesIdx >= 0) {
        parts.slice(1).forEach((v, idx) => {
          const ano = anosHeader[idx];
          if (!ano) return;
          if (/^[-ââ\.]+$/.test(v.trim()) || v.trim() === "") return;
      ...
      >
        ...
        <div>
          <p className="font-semibold text-sm text-gray-800 dark:text-gray-200">
            {status === "loading" ? "Processando..." : status === "done" ? fileName : status === "error" ? "Erro â tente novamente" : label}
          </p>
...
export default function PrevidenciarioPage() {
  ...
  const processarCnisOuCarta = async (file: File, tipo: "cnis" | "carta") => {
    ...
    try {
      ...
      toast({ title: "Erro na extraÃ§Ã£o", description: e.message, variant: "destructive" });
    }
  };
  ...
  const processarDtcCtc = async (file: File) => {
    ...
    try {
      ...
      if (dados.salarios.length === 0) {
        toast({ title: "Nenhuma remuneraÃ§Ã£o encontrada", description: "O documento pode ter formato diferente. Verifique se Ã© um CTC/DTC com Anexo V.", variant: "destructive" });
        setDtcCtcStatus("error"); return;
      }
      setDtcCtcData(dados); setDtcCtcStatus("done");
      toast({ title: `CTC/DTC extraÃ­do: ${dados.salarios.length} competÃªncias`, description: dados.nome || file.name });
    } catch (e: any) {
      setDtcCtcStatus("error");
      toast({ title: "Erro na extraÃ§Ã£o CTC/DTC", description: e.message, variant: "destructive" });
    }
  };

  // ConstrÃ³i tabela comparativa unificada
  ...
  const exportarCsv = () => {
    ...
    let header = "CompetÃªncia";
    if (hasCnis) header += ";CNIS";
    if (hasDtc) header += ";CTC/DTC";
    if (hasCnis && hasDtc) header += ";DiferenÃ§a";
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950">
      <header className="sticky top-0 z-20 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-bold text-gray-900 dark:text-white text-base">Importador PrevidenciÃ¡rio</h1>
            <p className="text-xs text-gray-500 dark:text-gray-400">CNIS Â· Carta Â· CTC/DTC</p>
          </div>
        </div>
        ...
        {abaAtiva === "documentos" && (
          <>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <UploadZone label="Importar CNIS" sublabel="PDF do Extrato PrevidenciÃ¡rio (Meu INSS)" icon={FileText}
              ...
              <UploadZone label="Importar Carta de ConcessÃ£o" sublabel="PDF da Carta de BenefÃ­cio do INSS" icon={FileUp}
              ...
              <UploadZone label="Importar CTC / DTC" sublabel="CertidÃ£o ou DeclaraÃ§Ã£o de Tempo de ContribuiÃ§Ã£o" icon={Scale}
            ...
            {!temDados && (
              <div className="text-center py-16 text-gray-400 dark:text-gray-600">
                ...
                <p className="text-sm">FaÃ§a o upload dos documentos acima para extrair os dados</p>
                <p className="text-xs mt-1">CNIS e Carta usam Gemini AI Â· CTC/DTC usam extraÃ§Ã£o local por coordenadas</p>
              </div>
            )}
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-semibold text-gray-700 dark:text-gray-300">
                    {cnisData.periodosContribuicao?.length || 0} perÃ­odos Â· {cnisData.salarios?.length || 0} competÃªncias
                  </span>
                </div>
                <SectionCard title="Dados do Segurado" icon={User}>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8">
                    ...
                    <InfoRow label="Nome da MÃ£e" value={cnisData.dadosSegurado?.mae} />
                  </div>
                </SectionCard>
                <SectionCard title="PerÃ­odos de ContribuiÃ§Ã£o" icon={Briefcase}>
                  {cnisData.periodosContribuicao?.length > 0 ? (
                    <div className="overflow-x-auto">
                      <table className="w-full text-xs">
                        <thead>
                          <tr className="border-b border-gray-200 dark:border-gray-700">
                            {["#","InÃ­cio","Fim","Empresa / DescriÃ§Ã£o","Natureza","CarÃªncia"].map(h => (
                  ...
                  ) : <p className="text-sm text-gray-500">Nenhum perÃ­odo encontrado.</p>}
                </SectionCard>
                {cnisData.salarios?.length > 0 && (
                  <SectionCard title={`SalÃ¡rios de ContribuiÃ§Ã£o (${cnisData.salarios.length} competÃªncias)`} icon={Banknote}>
                    <div className="overflow-x-auto max-h-64">
                      <table className="w-full text-xs">
                        <thead className="sticky top-0 bg-white dark:bg-gray-900">
                          <tr className="border-b border-gray-200 dark:border-gray-700">
                            <th className="text-left py-2 pr-8 font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">CompetÃªncia</th>
            ...
            {cartaData && (
              <div className="space-y-4">
                <div className="flex items-center gap-2">
                  <Badge className="bg-green-600 text-white text-xs px-3 py-1">CARTA DE CONCESSÃO</Badge>
                  <span className="text-sm font-semibold text-gray-700 dark:text-gray-300">{cartaData.especie}</span>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <SectionCard title="Dados do BenefÃ­cio" icon={Hash}>
                    <InfoRow label="NÃºmero" value={cartaData.numeroBeneficio} />
                    <InfoRow label="EspÃ©cie" value={cartaData.especie} />
                  ...
                  <SectionCard title="Valores" icon={Banknote}>
                    ...
                    <InfoRow label="SalÃ¡rio de BenefÃ­cio" value={cartaData.salarioBeneficio ? `R$ ${fmtBR(cartaData.salarioBeneficio)}` : undefined} />
                    <InfoRow label="Coeficiente" value={cartaData.coeficiente} />
                    <InfoRow label="Tempo de ContribuiÃ§Ã£o" value={cartaData.tempoContribuicao} />
                  </SectionCard>
                </div>
              </div>
            )}
            ...
            {dtcCtcData && (
              <div className="space-y-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-semibold text-gray-700 dark:text-gray-300">
                    {dtcCtcData.salarios.length} competÃªncias extraÃ­das
                  </span>
                </div>
                {(dtcCtcData.nome || dtcCtcData.cpf || dtcCtcData.orgao) && (
                  <SectionCard title="Dados do Servidor" icon={User}>
                    ...
                    <InfoRow label="ÃrgÃ£o" value={dtcCtcData.orgao} />
                  </SectionCard>
                )}
                  ...
                  <SectionCard title="PerÃ­odos de ContribuiÃ§Ã£o" icon={Briefcase}>
                    <div className="overflow-x-auto">
                      <table className="w-full text-xs">
                        <thead>
                          <tr className="border-b border-gray-200 dark:border-gray-700">
                            {["#","InÃ­cio","Fim","Cargo","Categoria"].map(h => (
                              <th key={h} className="text-left py-2 pr-4 font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">{h}</th>
                            ))}
                          </tr>
                        </thead>
                        <tbody>
                          {dtcCtcData.periodos.map((p, i) => (
                            <tr key={i} className="border-b border-gray-100 dark:border-gray-800">
                              ...
                              <td className="py-2 pr-4 text-gray-700 dark:text-gray-300">{p.cargo || "â"}</td>
                              <td className="py-2 text-gray-700 dark:text-gray-300">{p.categoria || "â"}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                ...
                <SectionCard title={`RemuneraÃ§Ãµes (${dtcCtcData.salarios.length} competÃªncias)`} icon={Banknote}>
                  <div className="overflow-x-auto max-h-64">
                    <table className="w-full text-xs">
                      <thead className="sticky top-0 bg-white dark:bg-gray-900">
                        <tr className="border-b border-gray-200 dark:border-gray-700">
          ...
          <>
            {!temComparativo ? (
              <div className="text-center py-16 text-gray-400 dark:text-gray-600">
                ...
                <p className="text-xs mt-1">CNIS + CTC/DTC Â· Identifica divergÃªncias competÃªncia a competÃªncia</p>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <h2 className="font-semibold text-gray-800 dark:text-gray-200">Comparativo por CompetÃªncia</h2>
                  ...
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="bg-gray-50 dark:bg-gray-800/60 border-b border-gray-200 dark:border-gray-700">
                        <th className="text-left px-4 py-3 font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide w-28">CompetÃªncia</th>
                        ...
                        {cnisData && dtcCtcData && <th className="text-right px-4 py-3 font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">DiferenÃ§a</th>}
                    ...
                    <tbody>
                      {comparativo.map((r, i) => {
                        ...
                        } else if (r.cnis == null) { status = "SÃ³ CTC/DTC"; statusClass = "text-purple-500"; }
                        else if (r.dtcCtc == null) { status = "SÃ³ CNIS"; statusClass = "text-blue-500"; }
                        ...
                        return (
                          <tr key={i} className={`border-b border-gray-100 dark:border-gray-800 ${rowClass}`}>
                            ...
                            {cnisData && <td className="px-4 py-2 text-right font-mono text-blue-700 dark:text-blue-300">{r.cnis != null ? fmtBR(r.cnis) : <span className="text-gray-300 dark:text-gray-600">â</span>}</td>}
                            {dtcCtcData && <td className="px-4 py-2 text-right font-mono text-purple-700 dark:text-purple-300">{r.dtcCtc != null ? fmtBR(r.dtcCtc) : <span className="text-gray-300 dark:text-gray-600">â</span>}</td>}
                            {cnisData && dtcCtcData && (
                ...
                {cnisData && dtcCtcData && (() => {
                  ...
                  return (
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                      {[
                        { label: "Com divergÃªncia", val: comDiff.length, color: "text-red-600 dark:text-red-400" },
                        { label: "Valores iguais", val: iguais.length, color: "text-green-600 dark:text-green-400" },
                        { label: "SÃ³ no CNIS", val: soCnis.length, color: "text-blue-600 dark:text-blue-400" },
                        { label: "SÃ³ no CTC/DTC", val: soDtc.length, color: "text-purple-600 dark:text-purple-400" },
                      ].map(({ label, val, color }) => (
                ...
                {cnisData && dtcCtcData && comparativo.filter(r => r.cnis != null && r.dtcCtc != null && Math.abs(r.cnis - r.dtcCtc) > 0.01).length > 0 && (
                  <div className="flex items-start gap-3 p-4 rounded-xl bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-700">
                    ...
                    <p className="text-xs text-amber-800 dark:text-amber-300">
                      Foram encontradas divergÃªncias entre os documentos. Exporte o CSV para anÃ¡lise detalhada ou encaminhe para comparaÃ§Ã£o no seu sistema de cÃ¡lculo.
                    </p>
                  </div>
                )}
              </div>
