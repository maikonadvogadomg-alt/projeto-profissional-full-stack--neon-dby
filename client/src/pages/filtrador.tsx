...
// ââ Storage key ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
// ââ Pure logic helpers âââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
function cleanMarkdownNoise(t: string, cfg: Pick<FilterCfg,
  ...
  if (cfg.cleanSymbols) {
    s = s.replace(/^\s{0,6}(?:[â¢\-ââ]+|#{1,6}|>+)\s+/gm, "");
    s = s.replace(/[#~]{2,}/g, " ");
  }
...
function splitParagraphs(t: string, splitLarge: boolean): string[] {
  const paragraphs = (t || "")
    .replace(/\r\n/g, "\n")
    .split(/\n{2,}|\n(?=\s*[A-ZÃÃÃÃÃÃÃÃÃÃÃÃ]{3,}[\s:])/g)
    .map(p => p.trim())
    .filter(Boolean);
...
function filterText(rawText: string, cfg: FilterCfg): FilterResult {
  ...
  for (const p of pars) {
    ...
    if (rule.drop) { removed.push({ motivo: `ExclusÃ£o por ${rule.reason}`, texto: p }); continue; }
...
export default function FiltradorJuridico() {
  ...
  const handleFiles = useCallback(async (files: FileList) => {
    ...
    setProcessing(true);
    setStatus("Importando arquivosâ¦");
    let combined = "";
    ...
    for (const file of Array.from(files)) {
      ...
      try {
        if (ext === "txt" || ext === "md") {
          ...
          setStatus(`Convertendo DOCX: ${file.name}â¦`);
          const mammoth = await import("mammoth");
          const buf = await file.arrayBuffer();
          const res = await mammoth.extractRawText({ arrayBuffer: buf });
          combined += (res.value || "") + "\n\n";
        } else if (ext === "pdf") {
          setStatus(`Extraindo PDF: ${file.name}â¦`);
          ...
          for (let i = 1; i <= totalPages; i++) {
            setStatus(`PDF ${file.name}: pÃ¡gina ${i}/${totalPages}â¦`);
            ...
            if (content.items.length > 0) {
              ...
              setStatus(`OCR pÃ¡gina ${i}/${totalPages}â¦`);
              try {
                // Carrega Tesseract.js via CDN somente quando necessÃ¡rio
                ...
                combined += `--- PÃ¡gina ${i} (OCR) ---\n${ocrResult.data.text}\n\n`;
              } catch {
                combined += `--- PÃ¡gina ${i} (Erro OCR) ---\nNÃ£o foi possÃ­vel extrair.\n\n`;
              }
            } else {
              combined += `--- PÃ¡gina ${i} ---\n[Sem texto extraÃ­vel â ative OCR para digitalizado]\n\n`;
            }
          }
        }
      } catch (e: any) {
    ...
    setStatus(`ImportaÃ§Ã£o concluÃ­da (${files.length} arquivo${files.length > 1 ? "s" : ""}).`);
    setProcessing(false);
  }, [useOCR, toast]);

  // ââ Run filter âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
  const runFilter = useCallback(() => {
    ...
    setTimeout(() => {
      try {
        ...
        setStats(res.stats);
        setStatus(
          `â Pronto â ${res.stats.kept} parÃ¡grafos mantidos, ` +
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <header className="border-b px-4 py-2 flex items-center gap-3 bg-card shrink-0">
        ...
        <span className="font-semibold text-sm">Filtrador JurÃ­dico</span>
        <span className="text-xs text-muted-foreground hidden sm:inline">
          Limpa repetiÃ§Ãµes, sujeira e prepara textos para a IA
        </span>
        {stats && (
        ...
        <div className="w-[400px] min-w-[320px] border-r flex flex-col overflow-y-auto bg-card/50">
          <div className="p-3 space-y-4 flex-1">
            ...
            <section>
              ...
              >
                ...
                {processing ? "Processandoâ¦" : "Selecionar TXT / PDF / DOCX / JSON / MD"}
              </Button>
            ...
            <section>
              ...
              <Textarea
                ...
                placeholder="Cole aqui o texto giganteâ¦ ou importe arquivos acima"
            ...
            <section>
              <h2 className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wide mb-2 flex items-center gap-1">
                <Settings className="w-3.5 h-3.5" /> ConfiguraÃ§Ãµes Gerais
              </h2>
              <div className="border rounded-md p-3 space-y-2">
                <div className="grid grid-cols-2 gap-1.5">
                  <CkBox id="cleanSymbols" label="Limpar sÃ­mbolos / markdown" checked={cleanSymbols} onChange={setCleanSymbols} />
                ...
                <div className="grid grid-cols-2 gap-2 mt-2">
                  <div>
                    <Label className="text-[10px] text-muted-foreground">MÃ­n. caracteres</Label>
                    <Input type="number" value={minChars} onChange={e => setMinChars(e.target.value)} className="h-7 text-xs" data-testid="input-min-chars" />
                  </div>
                  <div>
                    <Label className="text-[10px] text-muted-foreground">Similaridade (0.50â0.95)</Label>
                  ...
                  <div>
                    <Label className="text-[10px] text-muted-foreground">Shingle (2â5)</Label>
                    <Input type="number" value={shingle} onChange={e => setShingle(e.target.value)} className="h-7 text-xs" data-testid="input-shingle" />
                  </div>
                  <div>
                    <Label className="text-[10px] text-muted-foreground">Janela (parÃ¡grafos)</Label>
            ...
            <section>
              ...
              <div className="border rounded-md p-3 space-y-2">
                ...
                <Button size="sm" variant="ghost" className="text-[10px] h-6 px-2 text-red-500 w-full" onClick={() => { setBanWords(""); setBanRegex(""); }} data-testid="button-clear-ban-rules">
                  <Eraser className="w-3 h-3 mr-1" /> Limpar ExclusÃ£o
                </Button>
              </div>
            </section>
            ...
            <section>
              ...
              <div className="border rounded-md p-3 space-y-2">
                <div>
                  ...
                  <Textarea value={captureWords} onChange={e => setCaptureWords(e.target.value)} placeholder={"falsificaÃ§Ã£o\nestelionato"} className="min-h-[60px] text-xs font-mono resize-y mt-1" data-testid="textarea-capture-words" />
                </div>
                <div>
                  <Label className="text-[10px] text-muted-foreground">Destacar por REGEX</Label>
                  <Textarea value={captureRegex} onChange={e => setCaptureRegex(e.target.value)} placeholder={"\\b(falsificaÃ§Ã£o|fraude)\\b"} className="min-h-[40px] text-xs font-mono resize-y mt-1" data-testid="textarea-capture-regex" />
                </div>
            ...
            <section>
              <details className="border rounded-md">
                <summary className="px-3 py-2 text-xs font-medium cursor-pointer flex items-center gap-1.5">
                  <BookOpen className="w-3.5 h-3.5 text-muted-foreground" /> Guia RÃ¡pido de Regex
                </summary>
                <div className="px-3 pb-3 text-[11px] text-muted-foreground space-y-1.5 leading-relaxed">
                  <p><strong>\b</strong> â limite de palavra: <code className="bg-muted px-1 rounded">\bFraude\b</code></p>
                  <p><strong>^</strong> â inÃ­cio de linha: <code className="bg-muted px-1 rounded">^Modelo</code></p>
                  <p><strong>$</strong> â fim de linha: <code className="bg-muted px-1 rounded">deferimento$</code></p>
                  <p><strong>|</strong> â ou: <code className="bg-muted px-1 rounded">Fraude|Estelionato</code></p>
                  <p><strong>{"[\\s\\S]{0,500}"}</strong> â atÃ© 500 chars de contexto</p>
          ...
          <div className="sticky bottom-0 border-t bg-card p-3 space-y-2">
            ...
            <div className={`text-[11px] rounded-md px-3 py-2 border flex items-center gap-2 ${
              status.startsWith("â") ? "border-green-500/30 bg-green-500/10 text-green-700 dark:text-green-400"
              ...
              {processing ? <Loader2 className="w-3 h-3 animate-spin shrink-0" /> :
               status.startsWith("â") ? <CheckCircle2 className="w-3 h-3 shrink-0" /> :
          ...
          <div className="flex-1 overflow-y-auto p-4">
            {showRemoved ? (
              <div className="space-y-2">
                <p className="text-xs font-medium text-red-500 mb-3">{removed.length} parÃ¡grafos removidos:</p>
                ...
                {removed.length > 300 && (
                  <p className="text-xs text-muted-foreground text-center py-4">â¦ e mais {removed.length - 300} itens (exportar para ver todos)</p>
                )}
              </div>
            ) : outText ? (
              <pre
